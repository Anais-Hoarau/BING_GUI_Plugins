...
sub Run
    ...
    if ($numberOfFilesToBuild)
    {
        ...
        {
            my $parsedFileRef = NaturalDocs::Parser->ParseForBuild($file);
			my @parsedFile = @$parsedFileRef;
			$parsedFiles{ $file } = \@parsedFile;
        };
		
		NaturalDocs::TopicsTools->WorkOnParsedFiles(\%parsedFiles);
