...
classdef Loader_copie_simplifie < fr.lescot.bind.configurators.ConfiguratorUser
    ...
    methods(Access=protected)
        ...
        %}
        function resetPlugins(this)
            this.setPluginSet({});
            this.setPluginConfigurationSet({});
        end
        ...
        function saveEverythingToEnvironmentFile(this,filename)
            ...
            pluginsToSave = this.getPluginSet();
        ...
        %}
        function loadPluginsAndConfigurationsFromEnvironmentFile(this,filename)
        ...
        %}
        function loadTripsFromEnvironmentFile(this,filename)
    ...
    methods(Access=public)
        ...
        !
        %}
        function this =  Loader_copie_simplifie(varargin)
        ...
        function receiveConfiguration(this,pluginsId ,configuration)
            ...
            pluginConfigurationSet{pluginsId }=configuration;
            this.setPluginConfigurationSet(pluginConfigurationSet);
        end
        ...
        function launchPluginConfiguratorWithMetaInformations(this,pluginIndice,metaInformations,blank)
            ...
            pluginToConfigure = char(pluginSet{pluginIndice});
            existingConfiguration = pluginConfigurationSet{pluginIndice};
        ...
        function launchPluginsWithTrip(this,tripSelection)
            ...
            loadedPlugins = this.getPluginSet();
            ...
            switch loaderMode
                case 'union'
                    for i=1:length(loadedPlugins)
                        if ~this.isPluginConfigurationValid(i,trip.getMetaInformations())
            ...
            if ~allPluginsAreValid
                ...
                for i=1:1:length(loadedPlugins)
                    configurator = [char(loadedPlugins{i}) '.getConfiguratorClass()'];
                    pluginConfigurator = eval(configurator);
                    if ~isempty(pluginConfigurator)
                        pluginConfiguration = pluginsConfiguration{i};
                    ...
                    message = ['Launching plugin ' char(loadedPlugins{i})];
                    this.updateLaunchWindowLabel(message);
                    try
                        eval(expression);
        ...
        %}
        function out = addPluginWithConfiguration(this,pluginClassName,configuration)
            ...
            if any(strcmp(pluginClassName,availablePlugins(:)))
                ...
                % plugins
                loadedPlugins = this.getPluginSet();
                this.setPluginSet({loadedPlugins{:} pluginClassName});
                out = length(this.getPluginSet());
                % add configuration
        ...
        function out = getLoadedPluginsName(this,mode)
            ...
            loadedPlugins = this.getPluginSet();
            loadedConfigurations = this.getPluginConfigurationSet();
            if length(loadedPlugins)~= 0
                loadedPluginsName = cell(1,length(loadedPlugins));
                
                for i = 1:length(loadedPlugins)
                    % first cell is names
                    switch mode
                        case 'ClassName'
                            loadedPluginsName{i} = char(loadedPlugins{i});
                        case 'HumanReadableName'
                            expression = [ char(loadedPlugins{i}) '.getName()' ];
                            pluginName = eval(expression);
                            loadedPluginsName{i} = pluginName;
                    end
                end
        ...
        function out = isPluginConfigurationValid(this,pluginId,metaInformations)
            ...
            if ~this.isPluginConfigurationPresent(pluginId)
                ...
                loadedPlugins = this.getPluginSet();
                loadedPluginsName = loadedPlugins{pluginId};
        ...
        function out = isPluginConfigurationValidWithTripSet(this)
            ...
            for i=1:length(loadedPlugins)
                ...
                if ~this.isPluginConfigurationPresent(i)
                    ...
                    loadedPluginsName = loadedPlugins{i};
                    expression = [ char(loadedPluginsName) '.getConfiguratorClass()' ];
                    pluginConfigurator = eval(expression);
                    
                    loadedConfigurations = this.getPluginConfigurationSet();
                    pluginConfiguration = loadedConfigurations{i};
        ...
        %}
        function removePluginsAndConfigs(this,pluginIdsToDelete)
            loadedPlugins = this.getPluginSet();
            loadedConfigurations = this.getPluginConfigurationSet();
            loadedPlugins(pluginIdsToDelete) = [];
            loadedConfigurations(pluginIdsToDelete) = [];
    ...
    methods(Access=private)
        ...
        %}
        function out=getPluginSet(this)
            out = this.pluginSet;
        end
