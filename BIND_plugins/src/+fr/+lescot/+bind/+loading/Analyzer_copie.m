classdef Analyzer_copie < fr.lescot.bind.loading.Loader & fr.lescot.bind.plugins.TripPlugin
    ...
    methods(Access=public)
        ...
        %}
        function this=Analyzer_copie(trip)
            %%de magneto:
    ...
    methods(Access=private)
       ...
       %création d'une grade fenetre avec le magnéto en haut à gauche
        function buildUI(this)
            ...
            Figure = get(this.getFigureHandler());
            set(this.getFigureHandler, 'Name', 'Panel principal');
            set(this.getFigureHandler,'color','white');
            set(this.getFigureHandler,'Position',[windowWidth 0 windowWidth windowHeigth]);
           
            Figure= this.getFigureHandler(); %avec uifigure()
            Figure.set('Name','Panneau Plugin');
            Figure.set('Position', [0 0 windowWidth windowHeigth]);
            
            %disposer le magneto
            MagnetoPanel=uipanel(Figure,'position', [0 .50 .50 350], 'Title','Magneto','FontSize',12, 'BackgroundColor','red');
