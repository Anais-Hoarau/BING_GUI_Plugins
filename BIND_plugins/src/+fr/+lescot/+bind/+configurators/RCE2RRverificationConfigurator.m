%{
Class:
This class is the configurator of the <RCE2RRverificationConfigurator> plugin

%}
classdef RCE2RRverificationConfigurator < fr.lescot.bind.configurators.PluginConfigurator
    ...
    methods
        ...
        %{
        Function:
        The constructor of the RCE2CodingConfigurator plugin. When instanciated, a
        ...
        %}
        function this = RCE2RRverificationConfigurator(pluginId, metaTrip, caller, varargin)
    ...
    methods(Access = private)
        ...
        function buildWindow(this, metaTrip)
            ...
            set(this.getFigureHandler(), 'Name', 'RCE2Coding configurator');
    ...
    methods(Static)
        ...
        function out = validateConfiguration(referenceTrip,configuration)
            ...
            elseif ~referenceTrip.existData('MP150_data')
                warndlg('There is no MP150_data table in the selected trip. Make sure that the trip contain cardiac data to use this the RCE2 coding plugin')
                out = false;
            elseif ~referenceTrip.existEvent('cardiac_RRintervals')
                warndlg(['No RR intervals present in the trip. Make sure to run the script :' newline ...
                    ''' RCE2_cardiac_RRintervalsdectection'' before using this plugin'])
                out = false;
            end
        end
    end
