...
%}
classdef EventSituationDisplay < fr.lescot.bind.plugins.VisualisationPlugin
    ...
    methods
        ...
        %}
        function this = EventSituationDisplay(trip, situationIdentifiers, position, timeWindow)
            ...
            set(this.getFigureHandler(), 'Name', 'Afficheur d''events et situations');
            set(this.getFigureHandler, 'Position', [0 0 770 250]);
            %Positioning the window
            this.newPosition=get(this.getFigureHandler, 'Position')
            ...
            for i = 1:1:this.situationsNumber
                ...
                %text(5, yText, [this.situationName{i} '.' this.variableName{i}], 'Parent', this.legendAxesHandler, 'HorizontalAlignment', 'center', 'FontSize',10);
                %odette
                text(5, yText, this.situationName{i} , 'Parent', this.legendAxesHandler, 'HorizontalAlignment', 'center', 'FontSize',10);
                text(5, yText+ 0.2,  this.variableName{i}, 'Parent', this.legendAxesHandler, 'HorizontalAlignment', 'center', 'FontSize',10);
            end
      ...
      methods(Access = private)
            ...
            if ~isempty(situationsToDiplay)
                for i = 1:1:this.situationsNumber
                    ...
                    for j = 1:1:numberOfSegments
                        ...
                        if metaInfo.existSituation(this.situationName{i})
                            record = this.getCurrentTrip().getSituationOccurenceAtTime(this.situationName{i}, timecodes(1), timecodes(2));
                        elseif metaInfo.existEvent(this.situationName{i})
                            record = this.getCurrentTrip().getEventOccurenceAtTime(this.situationName{i}, timecodes(1));
                        end
                        ...
                        newText = text(mean(timecodes), yText, variableValue, 'Parent', this.axesHandler, 'HorizontalAlignment', 'center', 'FontSize', 12, 'Rotation', textAngle);
        ...
        function out = findSituationsToDisplay(this)
            ...
            for i = 1:1:this.situationsNumber
                ...
                elseif metaInfo.existEvent(this.situationName{i})
                    enclosedSituations = this.getCurrentTrip().getEventOccurencesInTimeInterval(this.situationName{i}, this.currentMinTime, this.currentMaxTime);
                    if ~enclosedSituations.isEmpty()
                        enclosedSituationsCellArray = enclosedSituations.buildCellArrayWithVariables({'timecode'});
                    else
                        enclosedSituationsCellArray = {};
                    end
                    %                     %Find all the situations ending in the window but not
                    %                     %starting in it.
                    %                     aroundLeftLimitSituations = this.getCurrentTrip().getEventOccurencesAroundTime(this.situationName{i}, this.currentMinTime);
                    %                     if ~aroundLeftLimitSituations.isEmpty()
                    %                         aroundLeftLimitSituationsCellArray = aroundLeftLimitSituations.buildCellArrayWithVariables({'timecode'});
                    %                     else
                    %                         aroundLeftLimitSituationsCellArray = {};
                    %                     end
                    %                     %Find all the situations starting in the window but not
                    %                     %ending in it.
                    %                     aroundRightLimitSituations = this.getCurrentTrip().getEventOccurencesAroundTime(this.situationName{i}, this.currentMaxTime);
                    %                     if ~aroundRightLimitSituations.isEmpty()
                    %                         aroundRightLimitSituationsCellArray = aroundRightLimitSituations.buildCellArrayWithVariables({'timecode'});
                    %                     else
                    %                         aroundRightLimitSituationsCellArray = {};
                    %                     end
                    situationsForThisVariable = [enclosedSituationsCellArray']; %; aroundLeftLimitSituationsCellArray'; aroundRightLimitSituationsCellArray'];
...
%         
    ...
    methods(Static)
        ...
        function out = getConfiguratorClass()
            out = 'fr.lescot.bind.configurators.EventSituationDisplayConfigurator';
        end
        ...
        function out = getName()
            out = '[S] Afficheur events et situations';
        end
