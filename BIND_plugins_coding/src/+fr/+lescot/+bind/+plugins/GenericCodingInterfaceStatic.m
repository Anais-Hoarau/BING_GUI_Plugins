...
%}
classdef GenericCodingInterfaceStatic < fr.lescot.bind.plugins.GraphicalPlugin & fr.lescot.bind.plugins.TripPlugin
    ...
    methods
        ...
        %}
        function this = GenericCodingInterfaceStatic(trip, protocol_fullpath)
            import fr.lescot.bind.utils.StringUtils;
            import fr.lescot.bind.coding.*;
            
        
            
            %prospect N = fr.lescot.bind.plugins.GenericCodingInterface.getNumberVariables(protocol_fullpath);
            
            this@fr.lescot.bind.plugins.TripPlugin(trip);
%prospect             this@fr.lescot.bind.plugins.MultiGraphicalPlugin(N);
            this@fr.lescot.bind.plugins.GraphicalPlugin();

            
%prospect             this.figureHandlers = this.getFigureHandlers;
            this.figureHandler = this.getFigureHandler;
          ...
          % odette temps = this.getCurrentTrip().getTimer().getTime()
...
%                 this.updateLists;
        ...
        function buildUI(this)
            this.pluginInterface = fr.lescot.bind.coding.InterfaceGraphiqueCodagePluginStatic(this.figureHandler, this.currentProtocol);
        ...
        function initializeTripTables(this)
            ...
            this.Variables.Table = this.currentProtocol.getName;
            for i=1:1:length(this.Variables.States)
                if this.getCurrentTrip.getMetaInformations.existSituationVariable(this.Variables.Table,this.Variables.States{i}.getName)
                    this.loadGenericStateTable(this.Variables.Table,this.Variables.States{i})
                else
                    this.createGenericStateTable(this.Variables.Table,this.Variables.States{i})
                end
            end
        ...
        function createGenericStateTable(this, TableName, stateVariable)
            VariableName = stateVariable.getName;
            
            newStateTable = fr.lescot.bind.data.MetaSituation;
            newStateTable.setName(TableName)
            newStateTable.setComments('')
            
            var{1} = fr.lescot.bind.data.MetaSituationVariable();
            var{1}.setName(VariableName)
            var{1}.setType('TEXT')
            
            if ~this.getCurrentTrip.getMetaInformations.existSituation(TableName)
                newStateTable.setVariables(var)
                this.getCurrentTrip.addSituation(newStateTable)
                this.getCurrentTrip.setIsBaseSituation(TableName,true)
            elseif ~this.getCurrentTrip.getMetaInformations.existSituationVariable(TableName,VariableName)
                this.getCurrentTrip.setIsBaseSituation(TableName,false)
                this.getCurrentTrip.addSituationVariable(TableName,var{1})
                this.getCurrentTrip.setIsBaseSituation(TableName,true)
            end
            ...
            if ~isempty(stateVariable.getDefaultModality)
                ...
                this.getCurrentTrip.setSituationVariableAtTime(TableName,VariableName,0,maxTime,defaultMod)
                this.getCurrentTrip.setIsBaseSituation(TableName,true)
            else
                errordlg(['Protocol Error: There is no default modality for the state variable :' VariableName])
            end
            
        end
        
        function loadGenericStateTable(this, TableName, stateVariable)
            name = stateVariable.getName;
            record = this.getCurrentTrip.getAllSituationOccurences(TableName);
            this.codedStateLists{end+1} = record.buildCellArrayWithVariables({'startTimecode','endTimecode',name});
        end
    ...
    methods(Static)
        ...
        function out = getName()
            out = '[ALL] Codage générique statique';
        end
