...
sub BuildContent #(sourceFile, parsedFile)
    ...
    while ($i < scalar @$parsedFile)
		...
		# We add a new group
		my $modifiersText = $accessName
						. ($modifiers{'sealed'} == 1 ? ' sealed' : '')
						. ($modifiers{'static'} == 1 ? ' static' : '')
						. ($modifiers{'abstract'} == 1 ? ' abstract' : '')
						. ($modifiers{'const'} == 1 ? ' const' : '');
        ...
        '<div class="C' . NaturalDocs::Topics->NameOfType($parsedFile->[$i]->Type(), 0, 1) . '">'
            . '<div class=CTopic' . ($i == 0 ? ' id=MainTopic' : '') . '>'
                ...
                . '<' . $headerType . ' class=CTitle>'
                    . '<a name="' . $anchor . '"></a>'
                    . $modifiersText . ' ' . $self->StringToHTML( $parsedFile->[$i]->Title(), ADD_HIDDEN_BREAKS)
                . '</' . $headerType . '>';
        ...
        if (defined $parsedFile->[$i]->Body())
            ...
            $output .= $self->NDMarkupToHTML( $sourceFile, $parsedFile->[$i]->Body(), $parsedFile->[$i]->Symbol(),
                                                                  ...
                                                                  $parsedFile->[$i]->Using(), undef);
            };
		if (defined $parsedFile->[$i]->Code())
		{
			$output .= $self->FormatCode( $sourceFile, '<code>' . $parsedFile->[$i]->Code() . '</code>', $parsedFile->[$i]->Symbol(),
                                                                  $parsedFile->[$i]->Package(), $parsedFile->[$i]->Type(),
                                                                  $parsedFile->[$i]->Using(), undef, $parsedFile->[$i]->Title());
		}
...
#   Function: FormatCode
#
#   Converts a block of code (code from a function) to HTML. Works like NDMarkupToHTML.
#
#   Parameters:
#
...
#       The text in HTML.
#
sub FormatCode #(sourceFile, text, symbol, package, type, using, style, title)
{
    my ($self, $sourceFile, $text, $symbol, $package, $type, $using, $style, $title) = @_;
    ...
    while (scalar @splitText)
	{
		...
		if ($text eq '<code>')
		{
			my $BTNTitle = 'BTN' . $title;
			$output .= '<br><blockquote><input id="'. $BTNTitle . '" type="button" 
						onclick="changeVisibility(\'' . $title . '\', \'' . $BTNTitle . '\')" 
						value="Display Code" />
						<pre class="hidden" id="' . $title . '">';
            $inCode = 1;
		}
		elsif ($text eq '</code>')
		{
...
#       The text in HTML.
#
sub NDMarkupToHTML #(sourceFile, text, symbol, package, type, using, style, title)
    {
    my ($self, $sourceFile, $text, $symbol, $package, $type, $using, $style, $title) = @_;
