...
sub OnComment #(string[] commentLines, int lineNumber, bool isJavaDoc)
	...
	my ($nativeKeyword, $nativeTitle) = NaturalDocs::Parser::Native->IsMine($commentLines, $isJavaDoc);
	my ($javadocKeyword, $javadocTitle) = NaturalDocs::Parser::JavaDoc->IsMine($commentLines, $isJavaDoc);
...
sub MergeAutoTopics #(language, autoTopics)
    ...
    while ($topicIndex < scalar @parsedFile && $autoTopicIndex < scalar @$autoTopics)
        ...
        # elsif ( !$topic->Title() || ($topic->Type() == $autoTopic->Type() && index($autoTopic->Title(), $cleanTitle) != -1) )
			...
			$topic->SetCode($autoTopic->Code());
			$topic->SetModifiers($autoTopic->Modifiers());
			$topic->SetParents($autoTopic->Parents());
