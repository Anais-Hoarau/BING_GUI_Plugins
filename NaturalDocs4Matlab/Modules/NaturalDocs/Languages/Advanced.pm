...
#	Returns:
#
#		tokensAndComments - An array contaning normalized tokenized code (like Token()) plus comment lines.
#
sub ParseForCommentsAndTokens #(FileName sourceFile, string[] lineCommentSymbols, string[] blockCommentSymbols, string[] javadocLineCommentSymbols, string[] javadocBlockCommentSymbols)
    ...
    while ($lineIndex < scalar @lines)
		...
		if ( ($isJavaDoc = $self->StripOpeningJavaDocBlockSymbols(\$line, $javadocBlockCommentSymbols)) ||
			...
			for (;;)
				...
				push @tokensAndComments, $originalLine . "\n";
			...
			if ($lineRemainder !~ /^[ \t]*$/)
			{
				# Remove the last line from @tokensAndComments. 
				pop @tokensAndComments;
				
				# If there was something past the closing symbol this wasn't an acceptable comment.
				if ($isMultiLine)
				{
					# Uglyness in its best way. In order to get the multiline comment in the code display, 
					# we get the first part of the line (i.e. the comment) as a commentLine (i.e. a block)
					# and we get the rest of the line tokenized.
				
					# We add "nothing" to @tokens just to keep the same indexing between it and @tokensAndComments
					push @$tokens, " ";
					
					# We get the commented part of the line
					$originalLine =~ s/($lineRemainder)//;
					
					# We add it to @tokensAndComments
					push @tokensAndComments, $originalLine;
					
					# We tokenize the rest of the line for both @tokens and @tokensAndComments

					$self->TokenizeLine($lineRemainder, \@tokensAndComments); 
				}
				else
				{
					...
					$self->TokenizeLine($lines[$lineIndex], \@tokensAndComments);
				};
		...
		# Retrieve single line comments.  This leaves $lineIndex at the next line.
		...
		elsif ( ($isJavaDoc = $self->StripOpeningJavaDocSymbols(\$line, $javadocLineCommentSymbols)) ||
			...
			do
				...
				push @commentLines, $line;
				push @$tokens, "\n";
				push @tokensAndComments, $lines[$lineIndex] . "\n";
			...
			while ($line !~ /^ *%{/ && $self->StripOpeningSymbols(\$line, $lineCommentSymbols));
        ...
        # Otherwise just add it to the code.
            ...
            {
            $self->TokenizeLine($line, \@tokensAndComments);
            $lineIndex++;
            };
...
sub TokenizeLine #(line)
    {
    my ($self, $line, $tokensAndComments) = @_;
    push @{$self->Tokens()}, $line =~ /(\w+|[ \t]+|.)/g, "\n";;
    push @$tokensAndComments, $line =~ /(\w+|[ \t]+|.)/g, "\n";
...
sub TryToSkipString #(indexRef, lineNumberRef, openingDelimiter, closingDelimiter, startContentIndexRef, endContentIndexRef)
    ...
    while ($$index < scalar @$tokens)
        ...
        if ($tokens->[$$index] eq "\\")
            {
			if ($tokens->[$$index+1] ne $closingDelimiter)
				{
