function batchExportSituationTSV_PROSPECT(TRIPS_FOLDER)
HORODATAGE = char(datetime('now','Format','yyMMdd_HHmm'));
TRIPS_FOLDER = '\\vrlescot\PROSPECT\CODAGE\TRIPSALL';
EXPORT_FOLDER = [TRIPS_FOLDER filesep '@FICHIERS_EXPORT' filesep 'Tableaux_codage' filesep HORODATAGE];
trip_files = dirrec(TRIPS_FOLDER, '.trip');
mkdir(EXPORT_FOLDER);
...
for i_situation = 1:length(situationsNames)
    ...
    for i_trip = 1:length(trip_files)
        ...
        exportTripSituation2TSVByTrip_PROSPECT(trip_file, file_id.(situation_name), situationsNames{i_situation})
    end
