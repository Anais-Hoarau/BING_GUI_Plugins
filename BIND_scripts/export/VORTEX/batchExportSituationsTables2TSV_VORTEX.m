function batchExportSituationsTables2TSV_VORTEX(fulldirectory)
    load('\\vrlescot\THESE_GUILLAUME\VORTEX\Fichier Indexation.mat');
    ...
    for i_situation = 1:length(situationsNames)
        ...
        for idx = 1:length(FichierIndexation)
            idx_part = FichierIndexation{idx, 1};
            idx_scenario = FichierIndexation{idx, 2};
            
            %% EXPORT TRIP SITUATIONS TO TSV FILES
            for i_trip = 1:length(trip_files)
                trip_file = trip_files{i_trip};
                if contains(trip_file, [idx_part '_' idx_scenario '_'])
