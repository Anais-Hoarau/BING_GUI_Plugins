...
dataName = 'MRTSynchroLookupTable';
...
pause(0.5);
disp('Starting Lookup table process');
for i=1:length(MRT_UTC_times)
    
    reportPeriod = 1000;
    % show progress report every "reportPeriod" lines
    if ~mod(i,reportPeriod)
        report = [sprintf('%.2f',(i /length(MRT_UTC_times)) * 100 ) ' % done.'];
        disp(report);
        pause(0.1);
    end
...
listeVariables = {'timecode' 'MRT_GyroZ_deg_per_hour' }; % timecode is system : mandatory
...
%theTrip.setBatchOfTimeDataVariablePairs(dataName,'MRT_UTC_time',timeValueCellArray);
