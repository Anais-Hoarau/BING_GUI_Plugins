function OrangeDRIVE_TOBII2BIND(xmlPath, pathToVAR, pathToTrip, trip_file)
...
% for i_data = 1:1:length(MetaDataNames)
    if MetaInformations.existData('tobii')
        trip.setIsBaseData('tobii', 0);
        trip.removeData('tobii');
    end
% end
delete(trip)
...
generateSQLiteFile_tobii(parsedXMLMappingFile, [pathToVAR filesep varFile], pathToTrip);