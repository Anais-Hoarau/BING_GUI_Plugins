function batch_import_SafeMoveSP2()
clear all;

%% LISTAGE DES DOSSIERS DES PARTICIPANTS

MAIN_FOLDER = '\\vrlescot.ifsttar.fr\SAFEMOVE SP2\DATA route\';
listing = dir(MAIN_FOLDER);
i=1;
while i <= length(listing)
    if listing(i,1).isdir && ~isempty(strfind(listing(i,1).name, 'Id'))
        if  strfind(listing(i,1).name, 'Id')==1
...
%     log_csv = fopen('log_import_SafeMoveSP2.csv', 'a+');
...
for i = 1:length(folder_lists)
    ...
    participant_name = full_directory(REG(end)+1:REG2(1)-1);                % identification participant (ex : Id914)
    ...
    [proc_mopad2struct, ~, ~] = magic_process('SafeMoveSP2_mopad2struct',trip,full_directory,participant_name);
    [proc_kvaser2struct, ~, ~] = magic_process('SafeMoveSP2_kvaser2struct',trip,full_directory,participant_name);
    
    %% Synchro des données Mopad et Kvaser
    [proc_syncMopad, ~, ~] = magic_process('SafeMoveSP2_sync_mopad_video',trip,full_directory,participant_name,'3brakes');
    [proc_syncKvaser, ~, ~] = magic_process('SafeMoveSP2_sync_kvaser_mopad',trip,full_directory,participant_name,'3brakes');
    
    %% Import des données Mopad et Kvaser dans le fichier trip
    [proc_import_mopad, time_import_mopad, ~] = magic_process('SafeMoveSP2_import_mopad2bind',trip,full_directory,participant_name);
    [proc_import_kvaser, time_import_kvaser, ~] = magic_process('SafeMoveSP2_import_kvaser2bind',trip,full_directory,participant_name);
    %[proc_import_kvaser_1, time_import_kvaser_1, ~] = magic_process('SafeMoveSP2_import_partiel_kvaser2bind_1',trip,full_directory,participant_name);
