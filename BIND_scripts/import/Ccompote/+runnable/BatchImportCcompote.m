...
file_id = fopen([CONFIG_FOLDER '\CCOMPOTE_FOLDERS.tsv'], 'w');
...
for i = 1:1:length(folders_list)
    ...
    if isdir([MAIN_FOLDER '\GROUPE_COLERE\' folders_list{i}]) && ~isempty(strfind(folders_list{i}, 'C')) && isempty(strfind(folders_list{i}, '@'))
        full_directory = [MAIN_FOLDER '\GROUPE_COLERE\' folders_list{i}];
        groupe_id = 'GROUPE_COLERE';
    elseif isdir([MAIN_FOLDER '\GROUPE_NEUTRE\' folders_list{i}]) && ~isempty(strfind(folders_list{i}, 'N')) && isempty(strfind(folders_list{i}, '@'))
    ...
    scenario_start_date = participant_id(strfind(participant_id, '2015'):strfind(participant_id, '2015')+7);
    scenario_start_time = participant_id(strfind(participant_id, '2015')+9:strfind(participant_id, '2015')+12);
    
    % identify needed files names
    trip_name = [participant_id '.trip'];
    simu_var_name = [participant_id '.var'];
    simu_xml_name = ['CCOMPOTE_' scenario_case '_Simu_Data_Mapping.xml'];
    ...
    if ~check_trip_meta(trip,'import_simu','OK')
        ...
        elseif ~exist(simu_xml_file, 'file')
            ...
            Ccompote_VAR2BIND(simu_xml_file, simu_var_file, full_directory, trip_file)
        end
    else
...
runnable.BatchIndicatorsCcompote(MAIN_FOLDER)

%% EXPORT SITUATIONS DATAS TO TSV FILE
runnable.batchExportTSV_CCOMPOTE(MAIN_FOLDER)
