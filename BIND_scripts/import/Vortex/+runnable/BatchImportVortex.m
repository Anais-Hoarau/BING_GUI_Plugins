...
%             trip = fr.lescot.bind.kernel.implementation.SQLiteTrip(trip_file, 0.04, true);
%             trip.setAttribute('import_questionnaires', '');
%             delete(trip);
        end
        ...
        import_questionnaires_needed = ~check_trip_meta(trip,'import_questionnaires','OK');
        delete(trip);
        
        import_simu_needed = 0;
        import_cardio_needed = 0;
        add_comments2Events_needed = 0;
        import_questionnaires_needed = 0;
        ...
        if add_comments2Events_needed
            disp('Ajout des commentaires en events...')
            addComments2Events(trip_file, 'variables_simulateur', 'commentaires')
        end
        
        %% IMPORT QUESTIONNAIRES DATA
        if import_questionnaires_needed
            disp('Import des données questionnaires...')
            import_questionnaires(trip_file)
        end
