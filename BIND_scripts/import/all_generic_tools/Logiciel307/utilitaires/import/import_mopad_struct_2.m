%% import_mopad_struct_2
%
% Ce script est tout a fait similaire au script 'import_mopad_struct'.
% L'unique changement vient du fait qu'il ne parse par le nom du dossier
% courant pour déterminer le nom et la date du sujet. Il utilise le
% fulldirectory pour determiner le nom et pouvoir savoir si c'est un aller ou un retour
% (Organisation des fichiers choisie la manip Atlas) et parse un des fichiers mopad .txt pour déterminer la
% date et l'heure de la manip
%
...
% converts the data into a Matlab structure.
%
% input argument:
...
%
function mopad = import_mopad_struct_2(full_directory)
    ...
    %find mopad .txt file
    txtFile =  fullfile(full_directory, '*.txt');
    listing_txt = dir(txtFile);
    
    for i=1:1:length(listing_txt)
        if ~strcmp('clap.txt',listing_txt(i).name)
           meta = regexp(listing_txt(i).name, '_', 'split');             
        end
    end
    
    %
    mopad.META = struct;
    mopad.META.nomSujet = participant_name;
    mopad.META.dateSujet = meta{end-1};
    mopad.META.MopadStarttime = meta{end}(1:end-4);
