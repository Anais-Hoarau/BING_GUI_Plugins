...
function import_data_struct_in_bind_trip(struct_to_import,trip,bind_prefix)
...
for i=1:length(data_names)
    ...
    if ~strcmp(data_names{i},'META') %|| isempty(strfind(data_names{26},'N_'))
        ...
        bind_timecode = num2cell(timecodes_sync');
        % for each variable
        for j=1:length(var_names)
            if ~strcmp(var_names{j},'time_sync')
                variable_struct = getfield(record,var_names{j});
                ...
                total_number_of_lines = length(bind_variable_values);
                ...
                for batch_num=0:(number_of_batches-1)
                    ...
                    batch_time_value_cell = [bind_timecode(index_batch) ; bind_variable_values(index_batch)];
