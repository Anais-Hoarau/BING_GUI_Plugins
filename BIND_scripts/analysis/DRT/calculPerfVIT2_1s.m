function [ output_args ] = calculPerfVIT2_1s(numsuj, track , typedrt, nametrip,idfileres, nametaches, namedebut , namefin)
...
for i=1:length(namedebut)
 ...
 %       dureeSeq = timecode(indexfin) - timecode(indexdebut);
         vitesse1s = interp1(timecode(indexdebut:indexfin),vitesse(indexdebut:indexfin),timecode(indexdebut):1:timecode(indexfin),'linear');
       for j=1:length(vitesse1s)
