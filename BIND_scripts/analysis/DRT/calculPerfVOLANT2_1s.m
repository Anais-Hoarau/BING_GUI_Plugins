function [ output_args ] = calculPerfVOLANT2_1s(numsuj, track , typedrt, nametrip,idfileres, nametaches, namedebut , namefin)
...
for i=1:length(namedebut)
 ...
 %       dureeSeq = timecode(indexfin) - timecode(indexdebut);
         anglevolant1s = interp1(timecode(indexdebut:indexfin),anglevolant(indexdebut:indexfin),timecode(indexdebut):1:timecode(indexfin),'linear');
       for j=1:length(anglevolant1s)
            fprintf(idfileres,'%f\t%s\t%s\t%s\t%f\n', numsuj, track , typedrt, nametaches{i}, anglevolant1s(j));
       end;
    end
