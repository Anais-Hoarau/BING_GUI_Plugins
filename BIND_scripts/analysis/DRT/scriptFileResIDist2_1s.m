...
pathToTrip = 'D:\Ldumont\DRT_MATLAB\datatripRoute';
listdir = dir(pathToTrip);
idfileres = fopen('fichierResIDist2_1s','w');
fprintf(idfileres,'IdSujet\tTrack\tTypeDRT\tCondition\tPk\tc.Pk\tinterdistance\n');

for i = 3: length(listdir)
   posstr = strfind(listdir(i).name, '.trip');
   if ~isempty(posstr)
        namefile = listdir(i).name; %01061358_S11AH.trip
        okfile = calculPerfIDist2_1s(str2num(namefile(11:12)), ... %numsujet
                namefile(13), ... % Track
                namefile(14), ... %TypeDRT
