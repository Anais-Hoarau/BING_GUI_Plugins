function BatchIndicatorsRCE2(MAIN_FOLDER)
...
%             trip.setAttribute('RRintervals','');
    ...
    event_xml_name = ['RCE2_' scenario_case '_Event_Mapping.xml'];
    situation_xml_name = ['RCE2_' scenario_case '_Situation_Mapping.xml'];
    event_xml_file = [CONFIGURATION_FOLDER '\FICHIERS_XML\' event_xml_name];
    situation_xml_file = [CONFIGURATION_FOLDER '\FICHIERS_XML\' situation_xml_name];
    ...
    add_RRIntervals_needeed = ~check_trip_meta(trip, 'RRdetection','1');
    delete(trip);
    ...
    try
        ...
        if add_event_needed || add_situation_needed
            RCE2AddEventsAndSituations(trip_file, event_xml_file, situation_xml_file)
        end
        
        if add_RRIntervals_needeed
            %RCE2CardiacRRIntervalsDetection(trip_file)
        end
        
        if add_indicators_needed
            RCE2AddIndicators(trip_file);
        end
        
    catch ME
        disp('Error caught, logging and skipping to next file');
        log = fopen('BatchIndicatorsRCE2.log', 'a+');
