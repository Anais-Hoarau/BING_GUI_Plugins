...
function ACT4_DATAPROCESSING_fab()
clear all
...
MAIN_FOLDER = 'E:\PROJETS ACTUELS\ACT\ACT4\Simulateur\Groupe 1';
...
for i_var = 1:1:length(list_var)
    ...
    for i_dt=1:1:length(data.commentaires)
        if ~isempty(strfind(data.commentaires{i_dt,3},'__SV'))
            ...
            % nom_columns_SV = {'Groupe' 'Nom Sujet' 'Scenario' 't_deb (s)' 't_fin (s)' 'duree (s)' 'surface (m²)' 'pic ang_vol (°/s²)' 'écart_max (m)' 'retour>centre_voie'};
