% Script de traitement de données pour l'expériementation ACT 4
function ACT4_DATAPROCESSING_fab3()
clear all
...
xls_filename = fullfile(MAIN_FOLDER,['ACT4_LOG_' date '_en_cours.xlsx']);

nom_columns_TR = {'Groupe' 'Nom Sujet' 'Scenario' 'Essai SV' 't_changement cap (s)' 't_deb (s)' 't_fin' 'TR (s)' 'Delta AngleVolant (°)'};
nom_columns_SV = {'Groupe' 'Nom Sujet' 'Scenario' 'Essai SV' 'duree_essai (s)' 't_debut_DT (s)' 't_fin_DT (s)' 'duree_DT (s)' 't_debut_SV (s)' 't_fin_SV (s)' 'duree_SV (s)' 'surface_SV (m²)' 'pic ang_vol (°/s²)' 'écart_max (m)' 'retour>centre_voie'};
nom_columns_id = {'Groupe' 'Nom Sujet' 'Scenario' 'ecart surface (m²)' 'nb sorties tot' 'nb sorties DT_SV' 'nb sorties DT_SS' 'nb sorties hors DT' 'durée sorties tot hors DT (s)' 'durée sorties moy hors DT (s)'};
...
%list_var ={'E:\PROJETS ACTUELS\ACT\ACT4\Simulateur\Groupe 2\S06\06051153_S06_G2_S4.var'};

for i_var = 1:1:length(list_var)
    %% Import des fichiers VAR
    tic
    data = import_var_ACT4(list_var{i_var});
    ...
    i_SV=0;
    i_SS=0;
    data.id_globaux.nbSortiesDT_SS = 0;
    for i_dt=1:1:length(data.commentaires)
        if ~isempty(strfind(data.commentaires{i_dt,3},'__SV'))
            ...
            %% VISUALISATION
            %             try
            %                 visualisation_ACT4(data, SV, indicateurs, message)
            %             catch
            %                 close all
            %             end
            %visualisation_ACT4(data, SV, indicateurs, message)
            ...
            % nom_columns_SV = {'Groupe' 'Nom Sujet' 'Scenario' 'Essai SV' 'duree_essai (s)' 't_debut_DT (s)' 't_fin_DT (s)' 'duree_DT (s)' 't_debut_SV (s)' 't_fin_SV (s)' 'duree_SV (s)' 'surface_SV (m²)' 'pic ang_vol (°/s²)' 'écart_max (m)' 'retour>centre_voie'};
            SV_data = {Group_sujet, Nom_sujet, data.scenario, commentaire_SV, indicateurs.essai.duree, ...
                indicateurs.DT.t_debut, indicateurs.DT.t_fin, indicateurs.DT.duree, indicateurs.SV.t_debut, indicateurs.SV.t_fin, indicateurs.SV.duree, indicateurs.SV.surface, indicateurs.SV.pic_angleVolant, indicateurs.SV.ecartMax/1000, indicateurs.SV.retour_state};
        ...
        if ~isempty(strfind(data.commentaires{i_dt,3},'__SS'))
            commentaire_SV = data.commentaires{i_dt,3};
            commentaire_SV = commentaire_SV(3:end);
            ...
            for i_fin=i_dt:1:length(data.commentaires)
                if ~isempty(strfind(data.commentaires{i_fin,3},'__FIN_TACHE'))
                    id = (data.commentaires{i_dt,1} : data.commentaires{i_fin,1} + 12*60); % +12sec de visu à la fin de la double tâche (12s x 60Hz)
                    t_finTache = data.commentaires{i_fin,2};
                    break
                end
            end
            ...
            mask_SV = data.essai.voie <= lim_voie_gauche | data.essai.voie >= lim_voie_droite;
            if  nnz(diff(mask_SV)) ~= 0
                data.id_globaux.nbSortiesDT_SS = data.id_globaux.nbSortiesDT_SS + 1;
            end
...
%             xlswrite(xls_filename,SV_data,'SV',xls_range(xls_column,xls_ligne,range_SV,1));
    ...
    % Comptage des sorties de voies contrôlées lors des doubles tâches
    data.id_globaux.nbSortiesDT_SV = i_SV - i_SS;
      
    %nom_columns_id = {'Groupe' 'Nom Sujet' 'Scenario' 'ecart surface (m²)' 'nb sorties tot' 'nb sorties DT_SV' 'nb sorties DT_SS' 'nb sorties hors DT' 'durée sorties tot (s)' 'durée sorties moy (s)'};
    ID_data = {Group_sujet, Nom_sujet, data.scenario, data.id_globaux.surface, data.id_globaux.nbSortiesTot, data.id_globaux.nbSortiesDT_SV, data.id_globaux.nbSortiesDT_SS, data.id_globaux.nbSortiesHorsDT, data.id_globaux.DureeSortiesHorsDT, data.id_globaux.DureeSortiesHorsDTmoy};
    ...
    function [data_out, SV, indicateurs, message, mask_SV] = calculerIndicateur(data, mask_essai)
        ...
        if SV.state
            ...
            if any(data_test_retour<0)
                indicateurs.SV.retour_state = true;
                ...
                indicateurs.SV.retour_state = false;
        ...
        indicateurs.DT.t_debut = data.essai.timecode(1);
        indicateurs.DT.t_fin = indicateurs.SV.t_debut;
        indicateurs.DT.duree = indicateurs.DT.t_fin - indicateurs.DT.t_debut;
        indicateurs.essai.duree = indicateurs.DT.duree + indicateurs.SV.duree;
    end
...
%% SUBFUNCTION VISUALISATION
    ...
    function visualisation_ACT4(data, SV, indicateurs, message)
        ...
        if SV.state
            ...
            if indicateurs.SV.retour_state
