...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            text(x+0.01*xrange + xOffset*xrange,y(1)+0.1*(y(2)-y(1)) + yrange*yOffset,label,'color',get(h,'color'),'Fontsize',9)
        else
            text(x-.05*xrange + xOffset*xrange,y(1)+0.1*(y(2)-y(1)) + yrange*yOffset,label,'color',get(h,'color'),'Fontsize',9)
        end
    end
