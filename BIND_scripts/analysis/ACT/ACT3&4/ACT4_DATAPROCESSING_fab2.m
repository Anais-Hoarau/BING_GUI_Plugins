...
function ACT4_DATAPROCESSING_fab2()
clear all
...
nom_columns_id = {'Groupe' 'Nom Sujet' 'Scenario' 'ecart surface (m²)' 'nb sorties tot' 'nb sorties DT_SV' 'nb sorties DT_SV_retour' 'nb sorties DT_SS' 'nb sorties hors DT' 'durée sorties tot (s)' 'durée sorties moy (s)'};
...
for i_var = 1:1:length(list_var)
        ...
        if ~isempty(strfind(data.commentaires{i_dt,3},'__SS'))
            ...
            for i_fin=i_dt:1:length(data.commentaires)
                if ~isempty(strfind(data.commentaires{i_fin,3},'__FIN_TACHE'))
                    id = (data.commentaires{i_dt,1} : data.commentaires{i_fin,1} + 12*60); % +5sec de visu à la fin de l'essai (5s x 60Hz)
    ...
    % Comptage des sorties de voies involontaires après SV contrôlées lors des doubles tâches
    data.id_globaux.nbSortiesDT_SV_retour
    data.id_globaux.nbSortiesDT_SS
    
    %nom_columns_id = {'Groupe' 'Nom Sujet' 'Scenario' 'ecart surface (m²)' 'nb sorties tot' 'nb sorties DT_SV' 'nb sorties DT_SV_retour' 'nb sorties DT_SS' 'nb sorties hors DT' 'durée sorties tot (s)' 'durée sorties moy (s)'};
    ID_data = {Group_sujet, Nom_sujet, data.scenario, data.id_globaux.surface, data.id_globaux.nbSortiesTot, data.id_globaux.nbSortiesDT_SV, data.id_globaux.nbSortiesDT_SV_retour, data.id_globaux.nbSortiesDT_SS, data.id_globaux.nbSortiesHorsDT, data.id_globaux.DureeSortiesHorsDT, data.id_globaux.DureeSortiesHorsDTmoy};
