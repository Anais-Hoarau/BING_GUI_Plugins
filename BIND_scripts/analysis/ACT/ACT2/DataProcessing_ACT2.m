...
%% REPERTOIRE DATA
REP = uigetdir();
%REP = 'D:\LESCOT\PROJETS\2013- ACT2\DATA2';
...
nom_columns = {'Nom Sujet' 'Scenario' 'Systeme' 'Type Essai'  'ligne deb essai' ' ligne fin essai' ...
               'ligne relachement acc' 'timecode relachement acc (s)' 'TR_acc (ms)' 'accelerateur deb essai'...
...
xls_filename =(fullfile(REP,['ACT2_data_' date '.xlsx']));

% Creation de l'entête des données aggrégées 
%xlswrite(xls_filename,nom_columns,xls_range(1,xls_ligne,range,1));
xlswrite(xls_filename,nom_columns);

xls_ligne =xls_ligne + 1;

%% GROUPES
GROUPE = {'Moyenne' 'Tard' 'Tot'}; % A MODIFIER EN FONCTION DES GROUPES
N_groupe = length(GROUPE);

for i_groupe=1:1:N_groupe
    ...
    for i_sujet=1:1:N_sujet
        %% RECUPERATION DES FICHIER .var A TRAITER
        listing =  dir ([REP_sujet{i_sujet} '\*.var']);
        FILE_var = cell(1,length(listing));
        for i_listing =1:1:length(listing)
            FILE_var{i_listing} = fullfile(REP_sujet{i_sujet}, listing(i_listing).name);
        end
        ...
        for i_var =1:1:N_var
            ...
            filtre_actif = true;
            n_largeur =4;
            [acc_filt] = filtrage_acc(acc,n_largeur,filtre_actif);
            
            %% CALCUL DES TEMPS DE REACTION PEDALE ACC   
            ReactionTime_acc = CalculerReactionTime_acc(timecode,acc,acc_filt,Essai);
