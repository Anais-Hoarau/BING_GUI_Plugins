function BatchIndicatorsVagabon2(MAIN_FOLDER)
...
%         trip.setAttribute('add_situations','');
    ...
    event_xml_name = ['VAGABON2_' scenario_case '_Event_Mapping.xml'];
    situation_xml_name = ['VAGABON2_' scenario_case '_Situation_Mapping.xml'];
    ...
    try
        ...
        if ~strcmp(scenario_id,'BASELINE')
            ...
            if add_event_needed || add_situation_needed
                Vagabon2AddEventsAndSituations(trip_file, event_xml_file, situation_xml_file)
            end
            
            if add_indicators_needed
                Vagabon2AddIndicators(trip_file);
            end
        ...
        log = fopen('BatchIndicatorsVagabon2.log', 'a+');
