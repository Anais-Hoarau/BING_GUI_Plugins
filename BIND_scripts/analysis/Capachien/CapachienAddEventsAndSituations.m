function CapachienAddEventsAndSituations(trip_file, event_xml_file, situation_xml_file)
...
if strcmp(scenario_id,scenario_cases{1})
    scenario_case = 'EXP';
    eventList = {'commentaire_centre', 'commentaire_periph'};
    situationList = {'detection_centre', 'detection_periph'};
end
...
switch scenario_case
    ...
    case 'EXP'
        ...
        addEventsAndSituation(trip, data_use, scenario_id, 'commentaire_centre', 'detection_centre', 'CentCrit')
        addEventsAndSituation(trip, data_use, scenario_id, 'commentaire_periph', 'detection_periph', 'P')
...
for i_com = 1:length(data_use.commentaires.simu)
    if ~isempty(strfind(data_use.commentaires.simu{i_com,3},['__' comment_name]))
        ...
        data_out.commentaire_var{i_var+1,1} = data_use.commentaires.simu{i_com+1,1};
        data_out.commentaire_var{i_var+1,2} = data_use.commentaires.simu{i_com+1,2};
        data_out.commentaire_var{i_var+1,3} = data_use.commentaires.simu{i_com+1,3};
...
%         else
            ...
            trip.setSituationVariableAtTime(situation_name, 'name', data_out.commentaire_var{i_var,2}, data_out.commentaire_var{i_var+1,2}, message_var(3:end));
