...
%   time of zero.
%
%   [...] = FINDPEAKS(...,'MinPeakHeight',MinPH) finds only those peaks that
%   are greater than the minimum peak height, MinPH. MinPH is a real valued
%   scalar. The default value of MinPH is -Inf.
%
%   [...] = FINDPEAKS(...,'MaxPeakHeight',MaxPH) finds only those peaks that
%   are less than the maximum peak height, MaxPH. MaxPH is a real valued
%   scalar. The default value of MaxPH is Inf.
%
%   [...] = FINDPEAKS(...,'MinPeakProminence',MinPP) finds peaks guaranteed
...
%
%   [...] = FINDPEAKS(...,'MaxPeakProminence',MaxPP) finds peaks guaranteed
%   to have a vertical drop of less than MaxPP from the peak on both sides
%   without encountering either the end of the signal or a smaller
%   intervening peak. The default value of MaxPP is Inf.
%
...
%   values it finds
%
%   FINDPEAKS(...,'autoScale',Sca) will adjust scale to the signal or not.
%
...
[y,yIsRow,x,xIsRow,minH,maxH,minP,maxP,minW,maxW,minD,minT,maxN,sortDir,annotate,refW,Sca] ...
...
iPk = removePeaksAboveMaxPeakHeight(y,iPk,maxH,refW);
iPk = removePeaksBelowThreshold(y,iPk,minT);

% indicate if we need to compute the extent of a peak
needWidth = minW>0 || maxW<inf || minP>0 || maxP<inf || nargout>2 || strcmp(annotate,'extents');
...
if needWidth
  ...
  % (wxPk)
  [iPk,bPk,bxPk,byPk,wxPk] = findExtents(y,x,iPk,iFinite,iInfite,iInflect,minP,maxP,minW,maxW,refW);
else
...
%--------------------------------------------------------------------------
function [y,yIsRow,x,xIsRow,Phmin,Phmax,Ppmin,Ppmax,Wmin,Wmax,Pd,Th,NpOut,Str,Ann,Ref,Sca] = parse_inputs(Yin,varargin)
...
defaultMaxPeakHeight = Inf;
defaultMinPeakProminence = 0;
defaultMaxPeakProminence = Inf;
...
if coder.target('MATLAB')
    ...
    addParameter(p,'MaxPeakHeight',defaultMaxPeakHeight);
    addParameter(p,'MinPeakProminence',defaultMinPeakProminence);
    addParameter(p,'MaxPeakProminence',defaultMaxPeakProminence);
    ...
    addParameter(p,'AutoScale',defaultAutoScale);
    parse(p,varargin{startArg:end});
    Phmin = p.Results.MinPeakHeight;
    Phmax = p.Results.MaxPeakHeight;
    Ppmin = p.Results.MinPeakProminence;
    Ppmax = p.Results.MaxPeakProminence;
    ...
    Sca = p.Results.AutoScale;
    
else
    parms = struct('MinPeakHeight',uint32(0), ...
                'MaxPeakHeight',uint32(0), ...
                'MinPeakProminence',uint32(0), ...
                'MaxPeakProminence',uint32(0), ...
    ...
    Phmin = eml_get_parameter_value(pstruct.MinPeakHeight,defaultMinPeakHeight,varargin{startArg:end});
    Phmax = eml_get_parameter_value(pstruct.MaxPeakHeight,defaultMaxPeakHeight,varargin{startArg:end});
    Ppmin = eml_get_parameter_value(pstruct.MinPeakProminence,defaultMinPeakProminence,varargin{startArg:end});
    Ppmax = eml_get_parameter_value(pstruct.MaxPeakProminence,defaultMaxPeakProminence,varargin{startArg:end});
    ...
    Sca = eml_get_parameter_value(pstruct.AutoScale,defaultAutoScale,varargin{startArg:end});
end
...
validateattributes(Phmin,{'numeric'},{'real','scalar','nonempty'},'findpeaks','MinPeakHeight');
validateattributes(Phmax,{'numeric'},{'real','scalar','nonempty'},'findpeaks','MaxPeakHeight');
validateattributes(Pd,{'numeric'},{'real','scalar','nonempty','nonnegative','<',x(M)-x(1)},'findpeaks','MinPeakDistance');
validateattributes(Ppmin,{'numeric'},{'real','scalar','nonempty','nonnegative'},'findpeaks','MinPeakProminence');
validateattributes(Ppmax,{'numeric'},{'real','scalar','nonempty','nonnegative'},'findpeaks','MaxPeakProminence');
...
Sca = validatestring(Sca,{'true','false'},'findpeaks','AutoScale');
...
%--------------------------------------------------------------------------
function iPk = removePeaksBelowMinPeakHeight(Y,iPk,Phmin,widthRef)
if ~isempty(iPk) 
  iPk = iPk(Y(iPk) > Phmin);
...
%--------------------------------------------------------------------------
function iPk = removePeaksAboveMaxPeakHeight(Y,iPk,Phmax,widthRef)
if ~isempty(iPk) 
  iPk = iPk(Y(iPk) < Phmax);
  if isempty(iPk) && ~strcmp(widthRef,'halfheight')
    if coder.target('MATLAB')
        warning(message('signal:findpeaks:largeMaxPeakHeight', 'MaxPeakHeight', 'MaxPeakHeight'));
    end
  end
end
...
%--------------------------------------------------------------------------
function [iPk,bPk,bxPk,byPk,wxPk] = findExtents(y,x,iPk,iFin,iInf,iInflect,minP,maxP,minW,maxW,refW)
...
% keep only those indices with at least the specified minimum prominence
[iPk,bPk,iLB,iRB] = removePeaksBelowMinPeakProminence(yFinite,iPk,bPk,iLB,iRB,minP);

% keep only those indices with at most the specified maximum prominence
[iPk,bPk,iLB,iRB] = removePeaksAboveMaxPeakProminence(yFinite,iPk,bPk,iLB,iRB,maxP);
...
%--------------------------------------------------------------------------
function [iPk,pbPk,iLB,iRB] = removePeaksAboveMaxPeakProminence(y,iPk,pbPk,iLB,iRB,maxP)
% compute the prominence of each peak
Ppk = y(iPk)-pbPk;

% keep those that are below the specified prominence
idx = find(Ppk <= maxP);
iPk = iPk(idx);
pbPk = pbPk(idx);
iLB = iLB(idx);
