classdef AnalyzerINTWP5 < fr.lescot.bind.loading.Loader
    ...
    methods(Access=private)
        ...
        function refreshTripUITable(this)
            ...
            if ~isempty(loadedTrips)
                ...
                % we load the UI with information from participants
                commonParticipant = this.getTripsCommonMetaInformations().getParticipant();
                commonParticipantAttributes = commonParticipant.getAttributesList();
                commonAttributesNumber = length(commonParticipantAttributes);
                ...
                for i=1:tripNumber
                    ...
                    for k = 1:commonAttributesNumber
                        ...
                        attributeValue = trip.getMetaInformations().getParticipant().getAttribute(commonParticipantAttributes{k});
                        data{i, k} = attributeValue;
                    end
                    try 
                        data{i,k+1} = trip.getAttribute('BeginDate');
                        data{i,k+2} = trip.getAttribute('EndDate');
                    catch
                        data{i,k+1} = 0;
                        data{i,k+2} = 0;
                    end
                ...
                commonAttributesList = {commonParticipantAttributes{:} 'Start Time' 'End Time'};
            else
                data = {};
                userData = {};
